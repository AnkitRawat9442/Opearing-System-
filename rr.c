...
// struct process {
//     int p_id ;
//     float  at , bt , ct, tat , rt;
// }p;

// int comparatorAT(const void *a , const void *b )
// {
//     float x = ((struct process *)a)->at;
//     float y = ((struct process *)b)->at;
//     if(x < y )
//       return -1 ;
//     else if(x >= y)
//      return 1;
...
//     for(int i=0;i<n;i++)
//      printf("%d\t\t%.2f\t%.2f\n",p[i].p_id,p[i].at,p[i].bt);

//     printf("\n"); 
   
//    qsort((void *)p , n , sizeof(struct process) , comparatorAT);
...
struct process {
  int psid , at , bt , ct , tat , wt , rt , start_time , bt_remaining ;
}ps[100];
...
int main ()
{
  ...
  int n  , tq;
  printf("Enter the number no fthe process : ");
  scanf("%d" ,&n);
 ...
 qsort((void *)ps , n , sizeof(struct process) , comparatorAt);
  ...
  while(completed != n)
  {
     index = queue[front];
     front++;
     if(ps[index].bt_remaining == ps[index].bt)    // pick process for the first time 
     {
       ps[index].start_time = findmax(ps[index].at , cur_time );
       cur_time = ps[index].start_time;
     ...
     {
       cur_time += ps[index].bt_remaining;
       ps[index].bt_remaining =  0 ;
       completed++;
       ps[index].ct = cur_time;
       ps[index].tat = ps[index].ct - ps[index].at;
       ps[index].wt = ps[index].tat - ps[index].bt;
     ...
     for( int i =  1 ;i< n ;i++)
     {
        if(ps[i].bt_remaining > 0 && ps[i].at <= cur_time && visited[i] == 0)
        {
          queue[++rare] = i;
          visited[i] =  1;
        }
  ...
  for(int i =0 ;i<n ;i++)
  {
     printf("Process %d\t%d\t%d\t%d\t%d\t%d\n" , ps[i].psid , ps[i].at , ps[i].bt , ps[i].ct , ps[i].tat , ps[i].wt);
  }
